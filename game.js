...
/* ============================================================
   MODEL SELECT (benchmark hub)
   Each entry is a build of GRID DEFENSE generated by one model.
   Add a new object here whenever a new run is showcased.
   - target 'splash' boots the build bundled in this page.
   - href '<url>' opens an externally hosted build in a new tab.
   - available:false renders a locked "coming online" card.
   ============================================================ */
const MODELS = [
  {
    id: 'claude-fable-5',
    ...
    desc: 'The showcase baseline build. A single-shot TRON tower defense.',
    available: true,
    target: 'splash',
  },
];
...
function renderModelCards() {
  ...
  MODELS.forEach(m => {
    ...
    if (m.available) card.onclick = () => launchModel(m);
...
function launchModel(m) {
  ...
  if (m.href) { window.open(m.href, '_blank', 'noopener'); return; }
  $('splash-model').textContent = `BUILD // ${m.name.toUpperCase()} (${m.id})`;
  show('splash');
}
...
$('btn-splash-models').addEventListener('click', (ev) => { ev.stopPropagation(); show('modelselect'); });
