...
const LEVELS = [
  {
    name: 'NEURAL PATHWAY',
    diff: 'EASY',
    desc: 'A single route through the cortex. Good for testing.',
  ...
  },
  {
    name: 'SYNAPSE GRID',
    diff: 'MEDIUM',
    desc: 'Branching pathways. Expect ambushes.',
  ...
  },
  {
    name: 'CORE ACCESS',
    diff: 'HARD',
    desc: 'Direct assault on the mainframe. Heavily defended.',
...
function renderLevelCards() {
  ...
  LEVELS.forEach((lv, i) => {
    ...
    for (let x = 0; x <= 15; x++) { mc.beginPath(); mc.moveTo(x * 188 / 15, 0); mc.lineTo(x * 188 / 15, 125); mc.stroke(); }
    for (let y = 0; y <= 10; y++) { mc.beginPath(); mc.moveTo(0, y * 125 / 10); mc.lineTo(188, y * 125 / 10); mc.stroke(); }
    mc.strokeStyle = '#00f6ff'; mc.lineWidth = 3; mc.shadowColor = '#00f6ff'; mc.shadowBlur = 6;
    mc.beginPath();
    lv.path.forEach(([c, r], j) => {
      const x = (c + 0.5) * 188 / 15, y = (r + 0.5) * 125 / 10;
...
function update(dt) {
  ...
  for (const e of G.enemies) {
    ...
    const a = G.pathPx[e.seg], b = G.pathPx[e.seg + 1];
    e.x = a.x + (b.x - a.x) * e.segT;
    e.y = a.y + (b.y - a.y) * e.segT;
    e.wob += dt * 6;
  }
  for (const e of G.enemies) {
    if (e.finished) {
...
function refreshPopup(t) {
  ...
  $('btn-sell').textContent = `RECYCLE ⚡${sellValue(t)}`;
}
